...
class ViewAccumulatedPromostar extends Component{
  constructor(props){
    super(props);
    this.state = {
      searchQuery: '',
      accumulatedPromoList: []
    }

  }

  componentDidMount(){
    fetch('http://localhost:1337/all-accumulated-promostar')
    .then((response) => {return response.json()})
    .then((result)=> {this.setState({accumulatedPromoList: result})})
    .then(()=>{console.log(this.state.accumulatedPromoList)})
  ...
  render(){
    return(
      ...
      <tbody>
          {
            this.state.accumulatedPromoList.map(
              (item, index)=>{
                return(
                  <tr key={index}>
...
export default ViewAccumulatedPromostar;